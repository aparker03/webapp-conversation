  ...
  return (
    <div className='space-y-5'>
      <div className='space-y-3'>
        <div className='inline-flex items-center rounded-full border border-[#E1D5C3] bg-white/80 px-3 py-1 text-xs font-semibold uppercase tracking-[0.08em] text-[#725329]'>
          {siteInfo.title}
        </div>
        <div>
        ...
        <div className='grid gap-2 tablet:grid-cols-2'>
          {examples.map(example => (
            <button
              type='button'
              key={example}
              className='rounded-lg border border-[#DED2C1] bg-white px-3 py-2.5 text-left text-sm leading-5 text-[#344054] shadow-sm transition hover:border-[#BFA783] hover:bg-[#FBFAF8] focus:outline-none focus:ring-2 focus:ring-[#8A642F]/30'
...
export const EditBtn = ({ className, onClick }: { className?: string, onClick: () => void }) => {
  ...
  return (
    <div
      className={cn('px-2 flex space-x-1 items-center rounded-md cursor-pointer focus:outline-none focus:ring-2 focus:ring-[#8A642F]/25', className)}
      onClick={onClick}
    >
